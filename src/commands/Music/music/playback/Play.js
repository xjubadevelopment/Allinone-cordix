...
class PlayCommand extends Command {
  ...
  _createSourceSelectionContainer(query, guildId, userId, searchResults = {}) {
    ...
    const youtubeEmoji = emoji.get("youtube", "▶️");
    const spotifyEmoji = emoji.get("spotify", "🎵");
    ...
    const content =
      ...
      `├─ **${emoji.get("info")} Query:** ${truncatedQuery}\n\n` +
      `${youtubeEmoji} **YouTube:**\n` +
      `└─ ${truncateTitle(ytTitle)}${ytAuthor ? ` - ${ytAuthor}` : ""}\n\n` +
      `${spotifyEmoji} **Spotify:**\n` +
      `└─ ${truncateTitle(spTitle)}${spAuthor ? ` - ${spAuthor}` : ""}\n\n` +
